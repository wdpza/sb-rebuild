...
export default function TextHeadingBlock({
  ...
  return (
    <div
      className={`relative py-24 bg-center ${bgUrl ? 'bg-cover' : ''}`}
      style={bgUrl ? { backgroundImage: `url(${bgUrl})` } : undefined}
    >
      {/* Optional overlay if you plan to add one later */}
      {/* <div className="absolute inset-0 bg-black/40" /> */}

      <div className="relative z-10 w-full max-w-[1400px] mx-auto flex flex-col text-neutral-softest px-6">
        {title ? (
        ...
        {sanitizedHtml ? (
          <div
            className="prose w-full text-neutral-softest mb-8"
